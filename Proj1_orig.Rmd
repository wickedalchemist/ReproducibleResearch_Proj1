...
The mean number of steps per day is `r round(daily_mean)` and the median number of steps per day is `r daily_median`.
...
On average, the `r unique_interval[interval_mean == max(interval_mean)]`th 5-minute interval has the most number of steps.
...
There are `r length(data$steps)` data points in the dataset, `r good` of which are real reported values, leaving `r length(data$steps)-good` missing values.  
...
The mean(median) number of steps taken for the dataset with replaced missing values is `r round(daily_mean_new)`(`r daily_median_new`). These values are extremely close to those computed from the dataset with missing values- replacing the missing values with the mean of the interval does not appear to significantly alter the distribution of the data. 
